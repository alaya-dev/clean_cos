<x-layouts.storefront title="Clean’Cos | Soins choisis avec intention" description="Découvrez les soins, produits de beauté et rituels Clean’Cos. Commandez en ligne avec paiement à la livraison en Tunisie.">
            ...
            <div class="home-hero-slides">
                @foreach($heroSlides as $slide)
                    <article class="home-hero-slide {{ $loop->first ? 'is-active' : '' }}" data-hero-slide aria-hidden="{{ $loop->first ? 'false' : 'true' }}">
                        ...
                        <div class="home-hero-copy"><p class="eyebrow">{{ $slide->eyebrow }}</p><h1>{{ $slide->heading }}</h1>@if($slide->supporting_text)<p>{{ $slide->supporting_text }}</p>@endif @if($slide->cta_label && $slide->cta_url)<a class="button button-light home-hero-cta" href="{{ $slide->cta_url }}">{{ $slide->cta_label }} <span aria-hidden="true">→</span></a>@endif</div>
                    </article>
                @endforeach
            </div>
    ...
    @if($reassuranceItems->isNotEmpty())
        @php($reassuranceIcons = ['ingredients_naturels' => 'ingredients-naturels.webp', 'teste_dermatologiquement' => 'teste-dermatologiquement.webp', 'livraison_rapide' => 'livraison-rapide.webp', 'paiement_livraison' => 'paiement-livraison.webp'])
        <section class="home-section reassurance-section" aria-labelledby="reassurance-title"><div class="home-section-heading"><p class="eyebrow">Nos engagements</p><h2 id="reassurance-title">Une expérience simple et rassurante</h2></div><div class="reassurance-grid">@foreach($reassuranceItems as $item)<article><span class="reassurance-icon" aria-hidden="true">@if(isset($reassuranceIcons[$item->icon_key]))<img src="{{ asset('images/reassurance/'.$reassuranceIcons[$item->icon_key]) }}" alt="" width="96" height="96" loading="lazy">@endif</span><h3>{{ $item->title }}</h3><p>{{ $item->text }}</p></article>@endforeach</div></section>
    @endif

    @if($socialItems->isNotEmpty())<section class="home-section social-section" aria-labelledby="social-title"><div class="home-section-heading"><p class="eyebrow">Suivez-nous</p><h2 id="social-title">L’univers Clean’Cos</h2></div><div class="social-rail">@foreach($socialItems as $item)<a href="{{ $item->url }}" rel="noopener noreferrer"><img src="{{ app(\App\Support\Media\PublicMediaUrl::class)->forPath($item->image_path) }}" alt="{{ $item->alt_text }}" width="600" height="600" loading="lazy"></a>@endforeach</div></section>@endif
