...
type Order = { public_reference: string; customer_name: string; customer_phone: string; status: string; total_millimes: number; created_at: string; items_count: number; is_returning_customer?: boolean; product_names?: string[]; product_thumbnail_url?: string | null; navex_delivery?: { status: string; label: string; tracking_code: string | null } | null; delivery?: { provider: 'navex' | 'first_delivery'; provider_label: string; status: string; label: string; tracking_code: string | null } | null };
...
type OrderListFilters = { search: string; product_public_id: string; product_name: string; status: string; navex_status: string; delivery_provider: string; archived: string; date_from: string; date_to: string; min_total_dt: string; max_total_dt: string; sort: string };
...
const OrdersView: Component = {
    ...
    setup() {
        ...
        const defaultFilters = (): OrderListFilters => ({ search: '', product_public_id: '', product_name: '', status: '', navex_status: '', delivery_provider: '', archived: '0', date_from: '', date_to: '', min_total_dt: '', max_total_dt: '', sort: '-created_at' });
        const filters = ref(defaultFilters());
        const draftOnly = computed(() => filters.value.archived === 'drafts');
        const showInlineDrafts = computed(() => draftOnly.value || (!filters.value.search && !filters.value.product_public_id && !filters.value.status && !filters.value.navex_status && !filters.value.delivery_provider && filters.value.archived === '0' && !filters.value.date_from && !filters.value.date_to && !filters.value.min_total_dt && !filters.value.max_total_dt));
        ...
        const load = async (requestedPage = 1, options: LoadOptions = {}) => {
            ...
            try {
                ...
                if (filters.value.delivery_provider) query.set('delivery_provider', filters.value.delivery_provider);
