...
const NavexView: Component = {
    ...
    template: `<section class="admin-page navex-page">
      ...
      <template v-else>
        ...
        <section class="navex-shipments"><header><div><p class="admin-eyebrow">Suivi des colis</p><h2>Expéditions Navex</h2><p>Les mises à jour automatiques n’effacent jamais le statut local sans correspondance confirmée.</p></div></header><div v-if="summary" class="navex-summary-grid" aria-label="Résumé des livraisons Navex"><article><small>En attente d’envoi</small><strong>{{ summary.pending_send }}</strong></article><article><small>En cours de livraison</small><strong>{{ summary.in_delivery }}</strong></article><article><small>Livrées aujourd’hui</small><strong>{{ summary.delivered_today }}</strong></article><article><small>Retournées</small><strong>{{ summary.returned }}</strong></article><article class="is-alert"><small>Actions requises</small><strong>{{ summary.action_required }}</strong></article></div><div class="navex-filters"><label class="toolbar-select"><span>Statut</span><SelectControl v-model="filters.status" :options="[{ value: '', label: 'Tous les statuts' }, { value: 'action_manuelle_requise', label: 'Action manuelle requise' }, { value: 'resultat_incertain', label: 'Résultat incertain' }, { value: 'acceptee_navex', label: 'Acceptée par Navex' }, { value: 'en_cours_livraison', label: 'En cours de livraison' }, { value: 'livree_payee', label: 'Livrée et payée' }, { value: 'retournee', label: 'Retournée' }]" @change="loadShipments(1)"/></label><label class="inline-check"><input v-model="filters.action_required" type="checkbox" @change="loadShipments(1)"> Actions requises uniquement</label></div><p v-if="loadingShipments" class="admin-loading">Chargement des expéditions…</p><div v-else-if="!page?.data.length" class="admin-empty"><strong>Aucune expédition Navex ne correspond aux filtres.</strong><span>Les commandes confirmées apparaîtront ici lorsqu’un colis sera mis en file.</span></div><div v-else class="navex-table"><div class="navex-table-head"><span>Commande</span><span>Suivi</span><span>Statut</span><span>Dernière synchronisation</span><span>Action</span></div><article v-for="shipment in page.data" :key="shipment.public_id"><div><strong>{{ shipment.order.public_reference }}</strong><small>{{ shipment.order.customer_name }}</small></div><code>{{ shipment.tracking_code || '—' }}</code><span class="admin-badge" :class="shipment.status === 'action_manuelle_requise' || shipment.status === 'resultat_incertain' ? 'is-danger' : ''">{{ shipment.display_status_label }}</span><time>{{ shipment.last_synchronized_at ? new Date(shipment.last_synchronized_at).toLocaleString('fr-TN') : 'Pas encore synchronisée' }}</time><RouterLink class="admin-outline" :to="'/orders/' + shipment.order.public_reference">Ouvrir</RouterLink></article><footer class="orders-pagination"><span>{{ page.total }} expédition{{ page.total > 1 ? 's' : '' }}</span><span>Page {{ page.current_page }} sur {{ page.last_page }}</span><div><button type="button" :disabled="page.current_page <= 1" aria-label="Page précédente" @click="loadShipments(page.current_page - 1)">‹</button><button type="button" :disabled="page.current_page >= page.last_page" aria-label="Page suivante" @click="loadShipments(page.current_page + 1)">›</button></div></footer></div></section>
      </template>
    </section>`,
};
