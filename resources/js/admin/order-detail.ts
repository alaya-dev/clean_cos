...
type Order = {
    ...
    customer_city: string; customer_governorate: string | null; first_delivery_locality_id: number | null; first_delivery_locality?: Locality | null; customer_address: string; customer_previous_order_at: string | null; is_exchange: boolean; exchange_article_designation: string | null; exchange_article_count: number | null; status: string; total_millimes: number; manual_total_millimes: number | null;
...
type FirstDeliveryShipment = { status: string; status_label: string; barcode: string | null; remote_state_code: number | null; remote_state: string | null; print_url: string | null; last_synced_at: string | null; last_error: string | null; status_history: { local_status: string; remote_state: string | null; recorded_at: string }[] };
type Detail = { order: Order; is_editable: boolean; is_delivery_editable: boolean; allowed_transitions: string[]; navex: { ready: { ready: boolean; reasons: string[]; mode: string }; shipment: NavexShipment | null; manual_update_required: boolean }; first_delivery: { ready: { ready: boolean; reasons: string[]; mode: string }; shipment: FirstDeliveryShipment | null; manual_update_required: boolean } };
...
type CustomerForm = { full_name: string; phone: string; city: string; governorate: string; first_delivery_locality_id: string; address: string; is_exchange: string; exchange_article_designation: string; exchange_article_count: number | null };
...
const OrderDetailView: Component = {
    ...
    setup() {
        ...
        const customer = ref<CustomerForm>({ full_name: '', phone: '', city: '', governorate: '', first_delivery_locality_id: '', address: '', is_exchange: 'Non', exchange_article_designation: '', exchange_article_count: null });
        ...
        const selectGovernorate = (value: string) => { selectedGovernorate.value = value; customer.value.governorate = value; customer.value.first_delivery_locality_id = ''; governorateQuery.value = value; governorateOpen.value = false; void loadLocalities(value); };
        ...
        const refresh = async () => {
            ...
            customer.value = { full_name: next.order.customer_name || '', phone: next.order.customer_phone || '', city: next.order.customer_city || '', governorate: savedGovernorate, first_delivery_locality_id: next.order.first_delivery_locality_id ? String(next.order.first_delivery_locality_id) : '', address: next.order.customer_address || '', is_exchange: next.order.is_exchange ? 'Oui' : 'Non', exchange_article_designation: next.order.exchange_article_designation || '', exchange_article_count: next.order.exchange_article_count ?? null };
            await loadLocalities(savedGovernorate);
        ...
        const refreshNavexStatus = async () => {
            ...
            if (!next?.navex || !next.first_delivery) return;
            detail.value.navex = next.navex;
            detail.value.first_delivery = next.first_delivery;
            detail.value.is_editable = next.is_editable;
            detail.value.is_delivery_editable = next.is_delivery_editable;
        };
        const navexNeedsFollowUp = () => ['en_attente_envoi', 'envoi_en_cours', 'resultat_incertain', 'acceptee_navex', 'en_attente_navex'].includes(detail.value?.navex.shipment?.status || '') || ['en_attente_envoi', 'envoi_en_cours', 'acceptee_first_delivery', 'en_attente_first_delivery'].includes(detail.value?.first_delivery.shipment?.status || '');
        ...
        return {
            ...
            governorates, governorateQuery, governorateOpen, governorateActiveIndex, selectedGovernorate, filteredGovernorates, selectGovernorate, selectExactGovernorate, localities, loadingLocalities, localityOptions,
            ...
            saveCustomer: () => detail.value && run(`orders/${detail.value.order.public_reference}`, 'PATCH', { lock_version: detail.value.order.lock_version, customer: { ...customer.value, first_delivery_locality_id: customer.value.first_delivery_locality_id || null } }, detail.value.navex.manual_update_required || detail.value.first_delivery.manual_update_required ? 'Informations mises à jour. Le colis existant doit être vérifié dans le dashboard du transporteur.' : 'Informations de livraison mises à jour.'),
            transition: async () => {
                if (!detail.value || !nextStatus.value) return;
                const status = nextStatus.value;
                const destructive = status === 'annulee';
                const navexMessage = status === 'confirmee' ? 'Si un transporteur est configuré en mode automatique, un seul colis sera mis en file. Une expédition existante ne sera jamais dupliquée.' : '';
            ...
            sendFirstDelivery: async () => { if (!detail.value || !await confirmAction('Envoyer ce colis à First Delivery ?', 'Les informations de livraison et le montant à encaisser seront transmis une seule fois.', 'Envoyer')) return; if (await run(`orders/${detail.value.order.public_reference}/first-delivery/send`, 'POST', { confirm_send: true }, 'Colis placé dans la file First Delivery.')) scheduleNavexRefresh(); },
            synchronizeFirstDelivery: async () => { if (!detail.value) return; if (await run(`orders/${detail.value.order.public_reference}/first-delivery/synchronize`, 'POST', {}, 'Synchronisation First Delivery demandée.')) scheduleNavexRefresh(); },
            retryFirstDelivery: async () => { if (!detail.value || !await confirmAction('Relancer l’envoi First Delivery ?', 'Utilisez cette action uniquement après un échec confirmé sans barcode.', 'Relancer')) return; if (await run(`orders/${detail.value.order.public_reference}/first-delivery/retry`, 'POST', { confirm_retry: true }, 'Le dossier First Delivery a été remis dans la file.')) scheduleNavexRefresh(); },
            cancelFirstDelivery: async () => { if (!detail.value || !await confirmAction('Annuler ce colis chez First Delivery ?', 'Cette action est disponible uniquement lorsque le fournisseur indique « En attente ».', 'Demander l’annulation', 'danger')) return; if (await run(`orders/${detail.value.order.public_reference}/first-delivery/cancel`, 'POST', { confirm_cancellation: true }, 'Demande d’annulation First Delivery envoyée.')) scheduleNavexRefresh(); },
        };
    },
